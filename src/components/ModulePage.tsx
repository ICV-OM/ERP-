...
export function ModulePage({module,locale}:{module:ModuleConfig;locale:Locale}){
  ...
  return <div>
    <section className="pageHeader"><div><div className="eyebrow">{t.module} · {module.accent}</div><h1>{module.title}</h1><p>{module.subtitle}</p></div><div className="pageActions">{module.key==="admin"&&<Link className="secondaryButton" href="/admin/users/new">{ar?"إنشاء مستخدم":"Create user"}</Link>}{module.key==="payroll"&&<Link className="secondaryButton" href="/payroll/generate">{ar?"توليد الرواتب":"Generate payroll"}</Link>}{first&&<Link className="primaryButton" href={`/${module.key}/${first.slug}`}>{ar?"فتح مساحة العمل":"Open workspace"}</Link>}</div></section>
